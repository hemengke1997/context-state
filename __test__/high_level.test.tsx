import React from 'react';
import { render, fireEvent, cleanup } from '@testing-library/react';
import { createContainer } from 'context-state';

interface Counter1Props {
  count: number;
  increment: React.DispatchWithoutAction;
}

interface Counter2Props {
  total: number;
  increment: React.DispatchWithoutAction;
}

describe('high level components spec', () => {
  afterEach(cleanup);

  it('should not render with other', () => {
    const CounterContainer = createContainer(() => {
      const [count, setCount] = React.useState(0);
      const [total, increment] = React.useReducer((c) => c + 1, 0);
      return {
        count,
        setCount,
        total,
        increment,
      };
    });

    const Counter1 = React.memo(({ count, increment }: Counter1Props) => {
      const renderCount = React.useRef(0);

      if (renderCount.current !== count) throw new Error('Inconsistent rendering (1)!');

      renderCount.current += 1;

      return (
        <div>
          <span>{count}</span>
          <button type="button" onClick={increment}>
            ADD1
          </button>
        </div>
      );
    });

    const Counter2 = React.memo(({ total, increment }: Counter2Props) => {
      const renderCount = React.useRef(0);

      if (renderCount.current !== total) throw new Error('Inconsistent rendering (2)!');

      renderCount.current += 1;
      return (
        <div>
          <span>{total}</span>
          <button type="button" onClick={increment}>
            ADD2
          </button>
        </div>
      );
    });

    const Counter1WithPicker = CounterContainer.withPicker(Counter1, ['count', 'increment']);
    const Counter2WithPicker = CounterContainer.withPicker(Counter2, ['total', 'increment']);

    const App = CounterContainer.withProvider(() => (
      <div>
        <Counter1WithPicker />
        <Counter2WithPicker />
      </div>
    ));

    const { getAllByText } = render(<App />);
    expect(() => fireEvent.click(getAllByText('ADD1')[0])).not.toThrow();
    expect(() => fireEvent.click(getAllByText('ADD2')[0])).not.toThrow();
  });
});
